...
public class PercentageGroup2Test extends AbstractJasperValueTest
{
	...
	protected void configureReport(final JasperReportBuilder rb)
	{
		...
		final TextColumnBuilder<Integer> column4 = col.column("Column4", "field4", Integer.class);
		final ColumnGroupBuilder group1 = grp.group(column1);
		final ColumnGroupBuilder group2 = grp.group(column2);
		final ColumnGroupBuilder group3 = grp.group(column3);
		...
		rb.setLocale(Locale.ENGLISH)
			.columns(
				...
				column3,
				column4,
				this.percentage1 = col.percentageColumn(column4),
				this.percentage2 = col.percentageColumn(column4).setTotalGroup(group2),
				this.percentage3 = col.percentageColumn(column4).setTotalGroup(group1),
				this.percentage4 = col.percentageColumn(column4).setTotalType(PercentageTotalType.REPORT))
			.groupBy(group1, group2, group3)
			.subtotalsOfPercentageAtGroupFooter(
				group3,
				this.subtotal1 = sbt.percentage(column4),
				this.subtotal2 = sbt.percentage(column4).setTotalGroup(group1),
				this.subtotal3 = sbt.percentage(column4).setTotalType(PercentageTotalType.REPORT))
			.subtotalsOfPercentageAtGroupFooter(
				group2,
				this.subtotal4 = sbt.percentage(column4),
				this.subtotal5 = sbt.percentage(column4).setTotalType(PercentageTotalType.REPORT))
			.subtotalsOfPercentageAtGroupFooter(group1, this.subtotal6 = sbt.percentage(column4));
	}
	...
	public void test()
	{
		...
		// percentage1
		this.columnDetailCountTest(this.percentage1, 24);
		this.columnDetailValueTest(
			...
			"34.78%");
		// percentage2
		this.columnDetailCountTest(this.percentage2, 24);
		this.columnDetailValueTest(
			this.percentage2,
			"4.76%",
			"9.52%",
			...
			"18.60%");
		// percentage3
		this.columnDetailCountTest(this.percentage3, 24);
		this.columnDetailValueTest(
			this.percentage3,
			"1.28%",
			"2.56%",
			...
			"10.81%");
		// percentage4
		this.columnDetailCountTest(this.percentage4, 24);
		this.columnDetailValueTest(
			this.percentage4,
			"0.33%",
			"0.67%",
		...
		// subtotal1
		this.subtotalIndexCountTest(this.subtotal1, 4, 8);
		this.subtotalIndexValueTest(
			this.subtotal1,
			4,
			"28.57%",
			"71.43%",
		...
		// subtotal2
		this.subtotalIndexCountTest(this.subtotal2, 5, 8);
		this.subtotalIndexValueTest(
			this.subtotal2,
			5,
			"7.69%",
		...
		// subtotal3
		this.subtotalIndexCountTest(this.subtotal3, 6, 8);
		this.subtotalIndexValueTest(
			this.subtotal3,
			6,
			"2.00%",
		...
		this.subtotalIndexValueTest(this.subtotal4, 2, "26.92%", "73.08%", "41.89%", "58.11%");
		
		// subtotal5
		this.subtotalIndexCountTest(this.subtotal5, 3, 4);
		this.subtotalIndexValueTest(this.subtotal5, 3, "7.00%", "19.00%", "31.00%", "43.00%");
		
		// subtotal6
		this.subtotalCountTest(this.subtotal6, 2);
		this.subtotalValueTest(this.subtotal6, "26.00%", "74.00%");
	}
