...
public class ComponentTransform
{
	...
	protected DRDesignComponent component(
		final DRIComponent component,
		...
		final DefaultStyleType defaultStyleType) throws DRException
	{
		...
		designComponent.setPrintWhenExpression(this.accessor.getExpressionTransform()
		...
		designComponent.setPrintInFirstWholeBand(this.accessor.getTemplateTransform()
			.getPrintInFirstWholeBand(component));
		designComponent.setPrintWhenDetailOverflows(this.accessor.getTemplateTransform()
			.getPrintWhenDetailOverflows(component));
		designComponent.setPrintWhenGroupChanges(this.accessor.getTemplateTransform()
		...
		for(final DRIPropertyExpression propertyExpression : component.getPropertyExpressions())
		{
			designComponent.getPropertyExpressions()
				.add(this.accessor.getExpressionTransform().transformPropertyExpression(propertyExpression));
		}
		...
		if(designTocHeading != null)
		{
			...
			final DRIDesignExpression anchorNameExpression =
				designTocHeading.getReferenceField().getAnchorNameExpression();
	...
	private void hyperlink(
		final DRDesignHyperlinkComponent designHyperlinkComponent,
		final DRIHyperLinkComponent hyperlinkComponent,
		final DRIReportStyle style,
		final boolean textStyle,
		final DefaultStyleType defaultStyleType)
		throws DRException
	{
		...
		{
			designHyperlinkComponent.setAnchorNameExpression(this.accessor.getExpressionTransform()
		...
		final DRDesignGroup resetGroup) throws DRException
	{
		...
		for(final DRIListCell innerComponent : list.getListCells())
		{
			...
			if(component instanceof DRIDimensionComponent)
			{
				...
				{
					horizontalAlignment =
						ConstantTransform.toHorizontalCellComponentAlignment(dimComponent.getWidthType());
				}
				if(verticalAlignment == null)
				{
					verticalAlignment =
						ConstantTransform.toVerticalCellComponentAlignment(dimComponent.getHeightType());
				}
			}
		...
		designList.setBackgroundComponent(this.listBackgroundComponent(
	...
	protected DRDesignComponent listBackgroundComponent(
		final DRIComponent backgroundComponent,
		final DefaultStyleType defaultStyleType,
		final ResetType resetType,
		final DRDesignGroup resetGroup) throws DRException
	{
		...
		{
			if(backgroundComponent instanceof DRIRectangle || backgroundComponent instanceof DRIImage
			...
			else
			{
				throw new DRDesignReportException(
					"List background component not supported. Only rectangle, image and textfield are supported");
			}
		}
		return null;
	}
	...
	private DRDesignSubreport multiPageList(final DRIMultiPageList multiPageList) throws DRException
	{
		...
		final DRIDesignExpression dataSourceExpression = this.accessor.getExpressionTransform()
	...
	protected DRDesignTextField textField(final DRITextField<?> textField, final DefaultStyleType defaultStyleType)
		throws DRException
	{
		...
		designTextField.setPatternExpression(this.accessor.getExpressionTransform()
			.transformExpression(textField.getPatternExpression()));
		designTextField.setHorizontalTextAlignment(templateTransform.getTextFieldHorizontalTextAlignment(
			textField,
			style));
		designTextField.setValueExpression(this.accessor.getExpressionTransform()
			.transformExpression(
				textField.getValueExpression(),
				templateTransform.getTextFieldValueFormatter(textField),
				null));
		designTextField.setMarkup(textField.getMarkup());
		if(textField.getEvaluationTime() != null)
		{
			designTextField.setEvaluationTime(ConstantTransform.textFieldEvaluationTime(
				textField.getEvaluationTime(),
				textField.getEvaluationGroup(),
				this.accessor));
			designTextField.setEvaluationGroup(
				this.accessor.getGroupTransform()
					.getGroup(ConstantTransform.textFieldEvaluationGroup(
						textField.getEvaluationTime(),
						textField.getEvaluationGroup(),
						this.accessor)));
		}
		else
		{
			if(textField.getEvaluationGroup() != null)
			{
				throw new DRException(
					"Evaluation group for textField is required only for evaluation time BEFORE_GROUP or GROUP");
			}
			...
			designTextField.setEvaluationGroup(this.detectEvaluationGroup(
				evaluationTime,
				designTextField.getValueExpression()));
		}
	...
	private DRDesignImage image(
		final DRIImage image,
		...
		final DefaultStyleType defaultStyleType) throws DRException
	{
		...
		designImage.setImageExpression(this.accessor.getExpressionTransform()
	...
	private DRDesignSubreport subreport(final DRISubreport subreport) throws DRException
	{
		...
		designSubreport.setReportExpression(this.accessor.getExpressionTransform()
			.transformExpression(subreport.getReportExpression()));
		designSubreport.setParametersExpression(this.accessor.getExpressionTransform()
			.transformExpression(subreport.getParametersExpression()));
		designSubreport.setConnectionExpression(this.accessor.getExpressionTransform()
			.transformExpression(subreport.getConnectionExpression()));
		designSubreport.setDataSourceExpression(this.accessor.getExpressionTransform()
	...
	private DRDesignList pageXofY(final DRIPageXofY pageXofY, final DefaultStyleType defaultStyleType)
		throws DRException
	{
		final TemplateTransform templateTransform = this.accessor.getTemplateTransform();
		...
		final HorizontalTextAlignment horizontalTextAlignment =
			templateTransform.getPageXofYHorizontalTextAlignment(pageXofY, style);
	...
	// total pages
	private DRDesignTextField totalPages(final DRITotalPages totalPages, final DefaultStyleType defaultStyleType)
		throws DRException
	{
	...
	// page number
	private DRDesignTextField pageNumber(final DRIPageNumber pageNumber, final DefaultStyleType defaultStyleType)
		throws DRException
	{
	...
	// current date
	private DRDesignTextField currentDate(final DRICurrentDate currentDate, final DefaultStyleType defaultStyleType)
		throws DRException
	{
		final CurrentDateExpression expression =
			new CurrentDateExpression(currentDate.getFormatExpression(), currentDate.getPattern());
	...
	protected DRDesignComponent booleanField(
		...
		final DRDesignGroup resetGroup) throws DRException
	{
		...
		if(component instanceof DRIImage)
		{
			...
			final TemplateTransform templateTransform = this.accessor.getTemplateTransform();
			((DRDesignImage)designComponent).setHorizontalImageAlignment(
				templateTransform.getBooleanHorizontalImageAlignment(
					booleanField,
					designComponent.getStyle()));
		}
		else
		{
	...
	protected DRDesignGenericElement genericElement(
		final DRIGenericElement genericElement,
		final ResetType resetType,
		final DRDesignGroup resetGroup) throws DRException
	{
		...
		for(final DRIParameterExpression parameterExpression : genericElement.getParameterExpressions())
		{
			designGenericElement.getParameterExpressions()
				.add(this.accessor.getExpressionTransform().transformParameterExpression(parameterExpression));
		}
		return designGenericElement;
	}
	
	// crosstab
	private DRDesignCrosstab crosstab(
	...
	// custom component
	private DRDesignComponent customComponent(
		final DRICustomComponent component,
		final ResetType resetType,
		final DRDesignGroup resetGroup) throws DRException
	{
		...
		final DRDesignComponent designComponent = (DRDesignComponent)componentTransfom.designComponent(
		...
		{
			designComponent.setHeight(this.accessor.getTemplateTransform()
	...
	private EvaluationTime detectComplexExpressionEvaluationTime(final DRIDesignComplexExpression complexExpression)
	{
		...
		for(final DRIDesignExpression expression : complexExpression.getExpressions())
		{
			...
			else if(evaluationTime != evalTime || evaluationTime.equals(EvaluationTime.GROUP) && evalTime.equals(
	...
	private DRDesignGroup detectEvaluationGroup(
		final EvaluationTime evaluationTime,
		final DRIDesignExpression expression)
	{
